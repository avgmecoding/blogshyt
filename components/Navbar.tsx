...
export default function Navbar() {
  ...
  return (
    // Max-width 5xl matches the home page content width for perfect alignment
    <nav className="w-full px-6 py-6 text-white max-w-5xl mx-auto z-50">
      <div className="flex items-center justify-between h-10">
        ...
        {/* Desktop Nav - Perfectly Centered in height */}
        <div className="hidden md:flex items-center gap-1 bg-zinc-900/40 p-1 rounded-xl border border-zinc-800/50 backdrop-blur-md">
          {navItems.map((item) => (
            <NavButton
              key={item.href}
              href={item.href}
      ...
      <div className={`md:hidden overflow-hidden transition-all duration-500 ease-in-out ${open ? "max-h-64 opacity-100 mt-4" : "max-h-0 opacity-0"}`}>
        <div className="flex flex-col gap-2 p-2 bg-zinc-900/90 backdrop-blur-xl border border-zinc-800 rounded-2xl">
          {navItems.map((item) => (
            <MobileLink
              key={item.href}
              href={item.href}
...
function MobileLink({ href, children, active, onClick }: { href: string; children: React.ReactNode; active: boolean; onClick: () => void }) {
  return (
    <Link href={href} onClick={onClick} className={`
      font-mono text-center text-[10px] uppercase tracking-widest py-4 rounded-xl border border-dashed transition-all duration-300
      ${active ? "border-orange-500/50 text-white bg-orange-500/10" : "border-zinc-800 text-zinc-500"}
    `}>
      {children}
    </Link>
  );
